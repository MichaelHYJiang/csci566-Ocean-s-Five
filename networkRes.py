...
def res_block(input, channel, prefix, i):
    
    #if input.shape[-1] != channel:
    #    input = slim.conv3d(input, channel, [1, 1, 1], rate=1, activation_fn=None, scope=prefix + 'g_input_res_rescale%d' %i)
    # else:
    #     input = slim.conv3d(input, channel, [1, 1, 1], rate=1, activation_fn=None, scope=prefix + 'g_input_res_up%d' %i)
    net = input
    net = slim.conv3d(net, channel, [3, 3, 3], rate=1, activation_fn=lrelu,
                      scope=prefix + 'g_res%d_conv1' % i)
    net = slim.conv3d(net, channel, [3, 3, 3], rate=1, activation_fn=None,
                      scope=prefix + 'g_res%d_conv2' % i)
    if input.shape[-1] != channel:
      # input = tf.concat([input, tf.zeros_like(input)], -1)
       input = slim.conv3d(input, channel, [1, 1, 1], rate=1, activation_fn=None, scope=prefix + 'g_input_res_up%d' %i)
    out = net + input
    return lrelu(out)
...
def network(input, depth=3, channel=32, prefix=''):
    ...
    # conv1 = res_block(input, channel, prefix, 1)
    ...
    conv2 = res_block(pool1, channel * 2, prefix, 2)
    ...
    conv3 = res_block(pool2, channel * 4, prefix, 3)
    ...
    if depth == 2:
        ...
        conv4 = res_block(pool3, channel * 8, prefix, 4)
        if depth == 3:
            ...
            conv5 = res_block(pool4, channel * 16, prefix, 5)
            ...
            conv6 = res_block(up6, channel * 8, prefix, 6)
        ...
        conv7 = res_block(up7, channel * 4, prefix, 7)
    ...
    conv8 = res_block(up8, channel * 2, prefix, 8)
    ...
    conv9 = res_block(up9, channel, prefix, 9)
